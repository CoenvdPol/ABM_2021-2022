...
to set-up
  ...
  set pmd-trashcan-size 5         ; assume that bins do not exceed
  set general-trashcan-size 10     ; assume that bins do not exceed
...
to change-perceptionlevel
  ifelse recycle-perception > (mean [recycle-perception] of other households in-radius 5)
     [ ifelse recycle-perception >= 0.99
        ...
        [set recycle-perception recycle-perception * 1.01 * bin-satisfaction ] ]
     [ ifelse recycle-perception <= 0.1
      [ ((((mean [recycle-perception] of other households in-radius 5) - recycle-perception)/(mean [recycle-perception] of other households in-radius 5))+ 1 ) * recycle-perception >= 1
...
to collect-waste ;;Either collection at home or central point collection --> This can be analysed
  if  counterpmd >= 2
      [ set pmd-collected [pmd-bin-level] of bin 0
        set pmd-bin-level 0
        set counterpmd 0]
  if  countergen >= 2
      [ set general-collected [general-bin-level] of bin 1
